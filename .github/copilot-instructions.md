...
This repository is a **prompt and tooling collection** for structured AI-assisted feature development.
It provides markdown prompt files that guide AI coding assistants through a PRD → Task List → Implementation workflow using TDD.
It also includes `ralph-wiggum`, a bash-based automation harness that executes task lists sequentially via GitHub Copilot CLI.
...
- `ralph-wiggum-v2.sh` — Bash script that reads a markdown task file and executes each task via GitHub Copilot CLI.
  Supports `--model`, `--max-retries`, `--system-prompt-file`, `--selfcorrect`, `--print-only`, and `--verbose` flags.
