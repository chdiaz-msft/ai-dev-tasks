...
1. **GraphQL vs REST for thread resolution** — GitHub's REST API for resolving PR review threads is limited. We may need to use the GraphQL API (`resolveReviewThread` mutation). Need to verify `gh api graphql` support.
2. **Review comment context** — should review agents receive just the diff, or also the full file contents for better context? (Diff-only is faster/cheaper; full files give better understanding.)
3. **Commit strategy** — should all fixes in one iteration be a single commit, or one commit per fix? (Single commit is simpler; per-fix commits make it easier to trace which fix addressed which comment.)
